...
export function ConfirmSheet({ open, onClose, icon: Icon, tone = "danger", title, desc, confirmLabel, onConfirm, cancelLabel = "Wróć" }) {
  if (!open) return null;
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 1600 }}>
      ...
      <div className="slideup" style={{ position: "absolute", left: 0, right: 0, bottom: 0, maxWidth: 680, margin: "0 auto", background: T.card2, borderRadius: "26px 26px 0 0", padding: "26px 22px 34px", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 12, color: T.sub, marginTop: 5, lineHeight: 1.5 }}>{desc}</div>
        <button
          onClick={onConfirm}
