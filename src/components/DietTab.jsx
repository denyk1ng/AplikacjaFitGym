    ...
    <div className="fu" style={{ background: T.card, border: `1px solid ${done ? `${color}55` : T.borderSoft}`, borderRadius: 22, padding: "14px 16px", marginBottom: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "1.3rem", color: done ? T.ok : color }}>
            {fmt(value)}
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end", height: 72 }}>
          {week.map((d, i) => (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, height: "100%" }}>
              ...
              <span style={{ fontSize: 9, fontWeight: 800, color: d.isToday ? T.accent : T.faint, fontFamily: "'Syne',sans-serif" }}>{d.label}</span>
            </div>
          ))}
        </div>
      </div>
